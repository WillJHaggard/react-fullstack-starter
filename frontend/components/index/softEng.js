...
import {SOFT} from "shared/constants";
...
import softEngItem from "frontend/components/item/softEng";

let dataCursor = state.select("softEngs");
...
export default class softEngIndex extends DeepComponent {
  render() {
      ...
      <DocumentTitle title="softEngs">
        <div>
          ...
          <section className="container">
            ...
            {pagination}
            <div className="row">
              {map(item => <softEngItem item={item} key={item.id}/>, items)}
            </div>
            {pagination}
          </section>
        </div>
  ...
  setOffset(offset) {
    dataCursor.set("offset", offset || SOFT.index.offset);
    actions.loadIndex();
  }
}
...
class Actions extends ShallowComponent {
  ...
  setLimit(limit) {
    dataCursor.set("limit", limit || SOFT.index.limit);
    actions.loadIndex();
  }
}
