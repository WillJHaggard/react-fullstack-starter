...
import actions from "frontend/actions/uxEng";
...
export default class uxEngEdit extends DeepComponent {
  handleBlur(key) {
    actions.validateEditForm(key).catch(err => null);
  }
    ...
    actions
      ...
      .then(item => {
        alertActions.addItem({
          message: "uxEng edited with id: " + item.id,
          category: "success",
        });
      })
      .catch(error => {
        alertActions.addItem({
          message: "Failed to edit uxEng: " + error,
          category: "error",
        });
      });
  }
              ...
              <div className="row">
                <div className="col-xs-12 col-sm-3">
                  <div className="thumbnail">
                    <img src={"http://flathash.com/" + item.id + "?size=200x200"} width="200px" height="200px"/>
                  </div>
                </div>
      ...
      <div className="actions">
        <div className="container">
          <div className="btn-group btn-group-sm pull-left">
            <Link to="uxEng-index" query={query} className="btn btn-gray-light" title="Back to list">
              <span className="fa fa-arrow-left"></span>
              <span className="hidden-xs margin-left-sm">Back to list</span>
            </Link>
          </div>
          <div className="btn-group btn-group-sm pull-right">
            <Link to="uxEng-add" className="btn btn-sm btn-green" title="Add">
              <span className="fa fa-plus"></span>
            </Link>
            <ItemLink to="uxEng-detail" params={{id: item.id}} className="btn btn-blue" title="Detail">
